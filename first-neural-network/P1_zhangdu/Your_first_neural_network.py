...
class NeuralNetwork(object):
    ...
    def train(self, features, targets):
        ...
        for X, y in zip(features, targets):
            ...
            final_inputs = np.dot(hidden_outputs,self.weights_hidden_to_output) # signals into final output layer (1,)
            ...
            error = y - final_outputs # Output layer error is the difference between desired target and actual output.(1,)
            
            output_error_term= error  #(1,)
            
            # TODO: Calculate the hidden layer's contribution to the error
            
            hidden_error = np.dot(output_error_term,self.weights_hidden_to_output.T)    #(2,)
            #hidden_error = np.dot(self.weights_hidden_to_output,output_error_term)
            # TODO: Backpropagated error terms - Replace these values with your calculations
            hidden_error_term = hidden_error * hidden_outputs * (1 - hidden_outputs)    #(2,)
...
# 模型的预测效果多数情况下比较符合，但是对于特殊情况比如圣诞节前后的预测就有偏高的情况。我推测其主要的问题在于样本的数据类型依然不够丰富，比如十二月末尾，大多数美国人会把圣诞节和元旦连起来休假，所以即使是工作日，他们也没有租车的需求。如果样本数据能够添加诸如 “平均出勤率”或“主要街道人流量”等数据，也许准确度能够进一步上升。
